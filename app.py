...
# API Keys
# ---------------------------------------------------------------------------
# Groq key: checked in this order so the same code works locally (.env) and
# on Streamlit Community Cloud (st.secrets), without ever asking the user.
#   1. st.secrets["GROQ_API_KEY"]   -> set in .streamlit/secrets.toml when deployed
#   2. GROQ_API_KEY env var         -> set in a local .env file
...
if uploaded_files:
    ...
    for uploaded_file in uploaded_files:
        ...
        os.remove(temppdf) # Clean up

    ## Split documents with reduced chunk size to avoid context errors
